...
gulp.task('clean:pot', function() {
	return del(['languages/convention-magician.pot']);
});
...
gulp.task('wp-pot', ['clean:pot'], function() {
	...
	.pipe(sort())
	.pipe(wpPot({
		domain: 'convention-magician',
		destFile:'convention-magician.pot',
		package: 'convention-magician',
		bugReport: 'http://missgeekbunny.studio',
		lastTranslator: 'Miss Geek Bunny Studios <missy@missgeekbunny.com>',
		team: 'Team <missy@missgeekbunny.com>'
	}))
...
gulp.task('watch', function() {
	...
	browserSync({
		...
		proxy: "testing.dev",         // Use http://missgeekbunny.studio:3000 to use BrowserSync
		watchOptions: {
